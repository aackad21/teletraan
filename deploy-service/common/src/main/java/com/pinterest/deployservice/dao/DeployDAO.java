...
public interface DeployDAO {
    ...
    List<DeployBean> getAcceptedDeploys(String envId, TimeInterval interval, int size)
            throws Exception;

    // Return ACCEPTED deploy whose suc_date is before before and build publish time is after after
    List<DeployBean> getAcceptedDeploysDelayed(String envId, TimeInterval interval)
            throws Exception;
