...
public class AutoPromoteDeployTest {
    ...
    public void testOneBadDeployPromote() throws Exception {
        ...
        badBuild.setCommit_date(now.minusHours(24).toInstant().toEpochMilli());
        badBuild.setPublish_date(now.minusHours(24).toInstant().toEpochMilli());
        ...
        prevDeploy.setStart_date(now.minusHours(23).toInstant().toEpochMilli());
        ...
        currentDeploy.setStart_date(now.minusHours(22).plusMinutes(40).toInstant().toEpochMilli());
    ...
    */
    ...
    public void testPredDeployAlreadyPromote() throws Exception {
        ...
        prevDeploy.setDeploy_id("deploy1");
        prevDeploy.setEnv_id(predEnvironBean.getEnv_id());
        prevDeploy.setStart_date(now.minusHours(26).toInstant().toEpochMilli());
        DeployBean currentDeploy = new DeployBean();
        currentDeploy.setDeploy_id("deploy2");
        currentDeploy.setFrom_deploy("deploy1");
        currentDeploy.setStart_date(now.minusHours(25).toInstant().toEpochMilli());
        allDeployBeans.add(prevDeploy);
        when(environDAO.getByStage(environBean.getEnv_name(), "pred")).thenReturn(predEnvironBean);
        when(deployDAO.getById("deploy1")).thenReturn(prevDeploy);
        when(deployDAO.getAcceptedDeploys(anyString(), any(), anyInt()))
    ...
    public void testPredDeployDelayPromote() throws Exception {
        ...
        prevDeploy.setDeploy_id("deploy1");
        prevDeploy.setEnv_id(predEnvironBean.getEnv_id());
        prevDeploy.setStart_date(now.minusMinutes(60).toInstant().toEpochMilli());
        DeployBean currentDeploy = new DeployBean();
        currentDeploy.setDeploy_id("deploy2");
        currentDeploy.setStart_date(now.minusMinutes(25).toInstant().toEpochMilli());
        currentDeploy.setFrom_deploy(prevDeploy.getDeploy_id());
        ...
        newDeploy.setStart_date(now.minusMinutes(6).toInstant().toEpochMilli());
        PromoteResult result =
                promoter.computePromoteDeployResult(environBean, currentDeploy, 1, promoteBean);
        assertEquals(PromoteResult.ResultCode.NoCandidateWithinDelayPeriod, result.getResult());
        ...
        newDeploy.setStart_date(now.minusMinutes(11).toInstant().toEpochMilli());
    ...
    public void testScheduledPromote() throws Exception {
        ...
        if (now.getHour() >= 10) {
            ...
            prevDeploy.setStart_date(cuttingPoint.minusHours(3).toInstant().toEpochMilli());
            currentDeploy.setStart_date(cuttingPoint.minusHours(2).toInstant().toEpochMilli());
            newDeploy.setStart_date(cuttingPoint.minusHours(1).toInstant().toEpochMilli());
            AutoPromoter promoter = new AutoPromoter(context);
            PromoteResult result =
                    promoter.computePromoteDeployResult(environBean, currentDeploy, 1, promoteBean);
            assertEquals(PromoteResult.ResultCode.PromoteDeploy, result.getResult());
            promoteBean.setDelay(
                    1 + (int) (now.toInstant().toEpochMilli() - newDeploy.getStart_date()) / 60000);
            result =
                    promoter.computePromoteDeployResult(environBean, currentDeploy, 1, promoteBean);
            assertEquals(PromoteResult.ResultCode.NoCandidateWithinDelayPeriod, result.getResult());
            ...
            prevDeploy.setStart_date(now.minusHours(27).toInstant().toEpochMilli());
            currentDeploy.setStart_date(now.minusHours(26).toInstant().toEpochMilli());
            newDeploy.setStart_date(now.minusHours(25).toInstant().toEpochMilli());
            AutoPromoter promoter = new AutoPromoter(context);
            PromoteResult result =
                    promoter.computePromoteDeployResult(environBean, currentDeploy, 1, promoteBean);
            assertEquals(PromoteResult.ResultCode.PromoteDeploy, result.getResult());
    ...
    public void testScheduledPromote2() throws Exception {
        ...
        if (now.getHour() >= 9 && now.getHour() <= 17) {
            prevDeploy.setStart_date(now.minusHours(25).toInstant().toEpochMilli());
            currentDeploy.setStart_date(now.minusHours(24).toInstant().toEpochMilli());
            newDeploy.setStart_date(now.minusHours(now.getHour()).toInstant().toEpochMilli());
            AutoPromoter promoter = new AutoPromoter(context);
            PromoteResult result =
                    promoter.computePromoteDeployResult(environBean, currentDeploy, 1, promoteBean);
            assertEquals(PromoteResult.ResultCode.PromoteDeploy, result.getResult());
